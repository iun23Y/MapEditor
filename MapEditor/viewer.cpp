		...
		for (const auto& entry : std::filesystem::directory_iterator(texturesPath)) {
			if (entry.path().extension() == ".png") {
				...
				if (img.loadFromFile(entry.path().string())) {
					textureImages[name] = std::move(img);
...
void SchematicViewer::buildTextures() {
	...
	sf::Image blockImage({ static_cast<unsigned int>(w * BLOCK_PIXEL_SIZE), static_cast<unsigned int>(l * BLOCK_PIXEL_SIZE) }, sf::Color(100, 100, 100));
	sf::Image heightImage({ static_cast<unsigned int>(w * BLOCK_PIXEL_SIZE), static_cast<unsigned int>(l * BLOCK_PIXEL_SIZE) }, sf::Color::Black);

	maxHeight = 0.0f;
	for (int x = 0; x < w; ++x)
		for (int z = 0; z < l; ++z)
			if (schem.topHeights[x][z] > maxHeight)
				maxHeight = static_cast<float>(schem.topHeights[x][z]);
	if (maxHeight < 1.0f) maxHeight = 1.0f;

	for (int x = 0; x < w; ++x) {
		for (int z = 0; z < l; ++z) {
			int h = schem.topHeights[x][z];
			if (h < 0) continue;

			sf::Image* texImg = getBlockTexture(schem.topBlocks[x][z]);

			for (int py = 0; py < BLOCK_PIXEL_SIZE; ++py) {
				for (int px = 0; px < BLOCK_PIXEL_SIZE; ++px) {
					sf::Color pixelCol;
					if (texImg) {
						pixelCol = texImg->getPixel({ static_cast<unsigned int>(px), static_cast<unsigned int>(py) });
					}
					else {
						pixelCol = getBlockColor(schem.topBlocks[x][z], schem.palette);
					}
					...
					float normH = static_cast<float>(h) / maxHeight;
					uint8_t heightValue = static_cast<uint8_t>(normH * 255.0f);
...
void SchematicViewer::handleEvents() {
	while (const std::optional event = window.pollEvent()) {
		if (event->is<sf::Event::Closed>()) window.close();
		if (const auto* resized = event->getIf<sf::Event::Resized>()) {
			windowWidth = resized->size.x; windowHeight = resized->size.y;
			window.setView(sf::View(sf::FloatRect({ 0, 0 }, { static_cast<float>(windowWidth), static_cast<float>(windowHeight) })));
		}
		...
		if (const auto* mousePressed = event->getIf<sf::Event::MouseButtonPressed>()) {
			if (mousePressed->button == sf::Mouse::Button::Left) {
				dragging = true;
				dragStart = sf::Vector2f(static_cast<float>(mousePressed->position.x), static_cast<float>(mousePressed->position.y));
...
void SchematicViewer::draw() {
	...
	if (ix >= 0 && ix < schem.width && iz >= 0 && iz < schem.length) {
		int bid = schem.topBlocks[ix][iz];
		int height = schem.topHeights[ix][iz];
		std::string name = "void";
		if (bid >= 0) {
			auto it = schem.palette.find(bid);
			if (it != schem.palette.end()) name = it->second;
		}
		coordText->setString("X:" + std::to_string(ix) + " Z:" + std::to_string(iz) +
			"  Y:" + std::to_string(height) + "  Block: " + name);
